...
class FinCalcuSupabaseUser extends BaseAuthUser {
  FinCalcuSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> finCalcuSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FinCalcuSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
